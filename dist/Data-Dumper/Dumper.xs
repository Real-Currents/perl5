...
/* This struct contains almost all the user's desired configuration. This
 * data is treated as constant by the recursive function, which has the
 * advantage of needing less memory than passing all of them on the stack
 * all the time. But this means that, for example, "sortkeys" is a separate
 * parameter. */
typedef struct {
    SV *pad;
    SV *xpad;
